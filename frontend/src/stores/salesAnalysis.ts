...
import type { ChartSpec, Message, MessageAttachment, SSEPayload } from '@/types'
import { useAuthStore } from '@/stores/auth'
import { errorFromResponse, toUserError } from '@/utils/safeError'

export type SalesFileStatus = 'pending' | 'parsing' | 'done' | 'failed'
...
export interface SalesWorkspaceMeta {
  id: number
  user_id: number
  title: string
  thread_id: string
...
function attachFromRows(
  rows: Array<{ role: string; content: string; attachments?: MessageAttachment[] }>,
): Message[] {
  return (rows || [])
    ...
    .map((r) => {
      ...
      for (const a of r.attachments || []) {
        ...
        if (kind === 'chart' || mime === 'application/x-echarts') {
          if (a.option && typeof a.option === 'object') {
            charts.push({
              chart_id: a.chart_id,
        ...
        } else if (kind !== 'xhs_card' && mime !== 'application/x-xhs-card') {
          files.push(a)
        }
      }
      return {
...
export const useSalesAnalysisStore = defineStore('salesAnalysis', () => {
  const ownerUserId = ref<number | null>(null)
  const workspace = ref<SalesWorkspaceMeta | null>(null)
  const files = ref<SalesFile[]>([])
  const tables = ref<SalesTableMeta[]>([])
  ...
  async function ensureWorkspace() {
    ...
    error.value = ''
    try {
      const res = await apiFetch('/api/sales-workspace/ensure', { method: 'POST' })
      if (!res.ok) throw new Error(await errorFromResponse(res, '无法打开销售分析区'))
      const data = (await res.json()) as {
        workspace: SalesWorkspaceMeta
        files: SalesFile[]
        tables: SalesTableMeta[]
      }
      ...
      error.value = toUserError(err, '无法打开销售分析区')
      ready.value = false
    }
  }
  ...
  async function loadMessages(tid: string) {
    try {
      ...
      messages.value = attachFromRows(rows)
    } catch {
      /* ignore */
    }
  }
  ...
  async function refreshFiles() {
    ...
    try {
      const res = await apiFetch(`/api/sales-workspace/${workspaceId.value}`)
      if (!res.ok) return
      const data = (await res.json()) as {
        files: SalesFile[]
        tables: SalesTableMeta[]
  ...
  async function upload(file: File) {
    ...
    try {
      ...
      const res = await apiFetch(`/api/sales-workspace/${workspaceId.value}/upload`, {
        method: 'POST',
        body: form,
      })
      if (!res.ok) throw new Error(await errorFromResponse(res, '上传失败'))
      const row = (await res.json()) as SalesFile
  ...
  async function removeFile(fileId: number) {
    ...
    try {
      const res = await apiFetch(
        `/api/sales-workspace/${workspaceId.value}/files/${fileId}`,
        { method: 'DELETE' },
      )
  ...
  async function sendMessage(text?: string) {
    ...
    if (!hasReadyFile.value) {
      error.value = '请先上传并等待 Excel 解析完成'
      return
    }
      ...
      const res = await apiFetch(
        '/api/chat',
        {
          ...
          body: JSON.stringify({
            ...
            message: trimmed,
            // 销售区走结构化工具，勿用 media_gen，避免意图路由当成「生图」
            intent: 'chat',
            sales_workspace_id: workspace.value.id,
          }),
        },
        { retry: false },
      )
  ...
  async function clearChat() {
    ...
    try {
      const res = await apiFetch(
        `/api/sales-workspace/${workspace.value.id}/clear-chat`,
        { method: 'POST' },
      )
